...
- `config` / `setup` / `pairing`: `telegram.json`, bot token setup, named bot/session profiles, authorization, env fallback, atomic persistence, effective config views, and live config accessors. `pairing` owns cryptographic single-use proof generation, non-rotating unexpired verifier reuse, salted verification, 10-minute expiry, atomic one-winner claims, and private profile-scoped attempt limiting; `config` validates/quarantines persisted verifier metadata, while `updates` broadly suppresses proof-shaped `/start <32hex>` text before public/default routing in both unpaired and paired states and delegates only strict eligible shapes to the claim parser.
- `recovery` / `inbound-recovery` / `menu-recovery`: `recovery` owns the strict recovery-v1 schema and profile-scoped filesystem store for inbound observation/admission, lightweight terminal dispositions, immutable admission revisions and proof verification, one committed-prefix authority, atomic grouped dispatch claims, full physical-byte quota reservation including verified materialized caches, SHA-256 payload/spool verification, private file rematerialization, automatic store-open/hourly retention, exact multi-family identity claims, collision-checked opaque recovery/reassignment handles, metadata-only status, and downgrade quarantine. `inbound-recovery` adapts responsibility-aware leader/follower admission, typed follower durable ACKs, strict replay envelopes, exact process-local session handoffs, queue materialization, file-path restoration, Pi dispatch fencing, lifecycle completion, and operator actions. `menu-recovery` owns the allowlisted metadata projection and confirmed retry/discard/safe-drain/reassignment/downgrade UI. `recovery` also owns the profile operation gate, while `bus-leader` coordinates exact follower fencing, runtime drain, store-exclusive recheck, and quarantine; `bus-follower` validates and applies exact authenticated fence/resume generations.
- `locks` / `polling`: extension-local transport owner storage, exact-owner epoch exposure, process-global reload generations, owner-aware polling lifecycle/takeover/follower registration, and the cohesive classic-vs-Threaded capability state/monitor/observation/polling orchestration. Polling also owns the single generation-bound long-poll supervisor/controller, bounded seeded restart policy, phase metadata, durable contiguous-prefix traversal, best-effort legacy config offset projection, and terminal transport-lease release hook wiring.
...
The runtime persistence surfaces have different authority and write pressure. Preserve that distinction when optimizing them:

- `recovery-v1*` is profile-scoped durable work state. Its private `0700` directory contains a `0600` transaction-safe snapshot plus `0600` minimum prompt payload and operation-owned attachment spool files. Every binary reference carries a required SHA-256 digest; reopen, drain, and idempotency paths verify both size and digest. Admission reserves complete snapshot/payload/spool bytes under the existing cross-process transaction, while spool restoration reserves its full projected physical cache bytes before publication and atomically refreshes `quota.reservedBytes`; fsynced binary publication fails before acknowledgement above 512 MiB. Every mutation reconciles temporary, orphaned, non-live, or corrupt materialized caches against live pre-dispatch/dispatching spool size+digest references. Unresolved and uncertain payload is retained without TTL; runtime-gated retention runs on store open and at a bounded hourly cadence, completed payload compacts after 24 hours and terminal inbound/outbound/bus metadata—including bus dedup—after 7 days within 100,000-record/64 MiB full-serialized-record bounds. `dispatching` becomes `execution-uncertain` after reopen and is never automatically replayed. Status/control surfaces use collision-checked opaque action ids, same-process handoff covers all three record families, and transfer/grant reassignment steps require a current binding plus authenticated exact-target callback. Downgrade requires zero nonterminal records, atomically renames the closed store to a quarantine backup, fsyncs the parent, and disables recreation until explicit restore/cancel. Recovery status is metadata-only and excludes payloads, attachment bytes/paths, chat/user ids, tokens, and bus secrets. The `/start` Recovery submenu exposes only profile/mode, unresolved family/state counts, quota use/limit, oldest age, fixed incident labels, required-action labels, and collision-checked opaque handles. Outbound `planned`, `pending`, `retryable-pending`, and live `sending` records appear as the metadata-only Pending delivery aggregate; `delivery-uncertain` records expose only opaque handle/family/state/action metadata. Confirmed Drain schedules only `planned`, `pending`, and non-exhausted `retryable-pending` work, never `sending` or uncertain work. Confirmed uncertain Retry warns that Telegram may already contain the effect and creates a linked attempt; Discard durably terminalizes outbound and linked inbound state before cleanup. Unsafe follower Bot API mutations use the same profile-scoped store as a durable bus journal: admission records the exact profile, target, request id, follower owner/registration/session authority, leader epoch/session, method, and payload fingerprint before execution. A completed response is replayed without repeating the mutation; a mutation started without a durable response becomes `bus-uncertain` after reopen and is never automatically replayed. The Recovery submenu exposes metadata-only opaque bus handles; confirmed Retry warns about possible duplication and creates a linked request, confirmed Discard terminalizes it, and safe Drain excludes uncertain work. Retry/discard/safe-drain, exact reassignment, and downgrade require confirmation. Reassignment refreshes the thread store and revalidates exact profile/target/owner/generation authority. Downgrade snapshots the current follower roster, closes the generation-bound local gate, obtains exact authenticated follower fence ACKs, stops polling and new forwarding/registration, suspends grouped input and queue dispatch, awaits in-flight drain, rechecks all durable families under store-exclusive mode, then atomically quarantines and parent-fsyncs the store. Pre-quarantine failures resume only after any changed store-exclusive mode is cancelled and each runtime resume succeeds while its exact fence remains closed; rename success or uncertainty leaves all gates closed, and the on-disk mode remains the final racing-process fence.
...
- Runtime diagnostics are append-only profile+instance segments with a unique writer generation and process-generation lease; session reload explicitly retires the old writer before starting its replacement, and scope changes or rotations append boundary records instead of resetting shared files. Each segment retains an immutable creation timestamp and even low-volume active writers rotate once that creation age exceeds 14 days. Retention is exactly 14 days, 100 segments, and 100 MiB per profile including live segments. Stale leases are reclassified through process and exact-generation liveness; dead segments may be removed under the profile transaction only after their lease is successfully removed, while lease cleanup failure retains the segment in quota and records a diagnostics incident. Live segments are never deleted, and capacity overflow drops new diagnostics while retaining an in-memory incident. Directories/files use `0700`/`0600` where supported, and persisted values remain redacted.
...
`tmp/telegram/logs*.segment-*.jsonl` is profile+instance-scoped redacted runtime evidence. Each writer owns a unique generation segment and process-generation lease; reload retires the prior generation explicitly, and lifecycle changes append boundaries rather than resetting shared evidence. Segments remain diagnostics only and never become routing/provisioning authority.
...
### Durable Outbound Lifecycle

For Telegram-owned active turns, semantic completion crosses a synchronous durable handoff before the agent lifecycle resets the active turn. The handoff transforms the answer, plans ordered final-text/media/voice/Guest units, captures operation-owned sources, commits the outbound intent, activates it as pending, and only then releases the inbound dispatch lease. The outbound worker serializes one unit at a time behind the profile operation gate. A confirmed response is persisted as a unit receipt before another unit starts; the final receipt atomically marks the outbound record delivered, completes linked inbound sources, and releases outbound spools. Queue advancement is notified only after a durable terminal disposition.

Reopen is conservative. `planned`, `pending`, and bounded `retryable-pending` records are eligible to resume from the first unreceipted unit. A pre-commit semantic answer remains volatile: if the process dies before the outbox commit, the linked inbound dispatch reopens as `execution-uncertain` and no answer is reconstructed or auto-executed. A `sending` record reopens as `delivery-uncertain`; response-lost and confirmed-before-receipt failures reach the same state directly. These ambiguous cases never auto-resend. A delivered record and its completed inbound sources are reconciled without transport replay, so a later safe turn may dispatch once after restart.

This protocol provides durable disposition and bounded replay, not exactly-once Telegram effects. Telegram and the local snapshot do not share a transaction, so confirmed operator retry of uncertain work explicitly accepts possible duplication and creates a linked attempt. Payloads, receipts, and attachment spools remain in the private quota-accounted recovery store under its 24-hour payload/7-day terminal-metadata retention policy; uncertain material has no TTL. Metadata status never exposes the retained answer, bytes, path, target identity, or internal ids. Profile downgrade fences local and exact follower generations, drains workers, and requires zero nonterminal outbound records before quarantine.
...
Telegram status rendering distinguishes connected, active, dispatching, queued, tool-running, model-switching, and compacting states. If a queue mutation removes the last waiting item while Telegram-owned work still has running tools, status remains active instead of degrading to connected. Pi's terminal `telegram` status key is always cleared so these bridge states remain on Telegram-owned surfaces.
...
`/telegram-status` records grouped diagnostics for transport/API, polling/update, prompt dispatch, controls, typing, compaction, setup, session lifecycle, attachment queue/delivery, and recent redacted runtime events. Expected preview noise such as unchanged edit responses is filtered out. The Pi terminal `telegram` status key remains cleared; detailed failure text stays in Telegram diagnostics and profile-scoped logs.
...
`ASYNC-POLL-01` is owned by the polling supervisor: one generation owns exactly one controller/loop, bootstrap/non-conflict API/body-parse/persistence/retry-sleep exits are phase-classified, and bounded seeded jitter governs restarts; `409` conflict draining plus handler retry/poison handling remain loop-local. Explicit shutdown or ownership loss never restarts. Missing/invalid configuration and Bot API `401`/`403` terminalize immediately; restart-budget exhaustion marks that exact generation terminalizing, blocks replacement starts until teardown settles, synchronously stops ownership heartbeats, then stops polling and bus health/pruning/server through the existing lifecycle before releasing the exact transport lease. Both the controller and terminal lease handler recheck generation authority so a stale callback cannot stop a newer poller. Diagnostic details contain only phase, restart count, generation, and timestamps. Telegram API attempts independently compose caller cancellation with bounded method-class deadlines through response-body consumption; `getUpdates` adds fixed network grace to its requested long-poll timeout.
...
### P1 Async Boundary Closure

- `ASYNC-TEXT-01`: configured text transforms remain fallback-safe, while public text handlers execute in registration order and each rejection is isolated. Diagnostics expose only the generated handler id and `inbound:text` category, never the thrown message or input.
- `ASYNC-MEDIA-01`: every Telegram download receives an operation-owned cleanup capability. Partial download/build failures clean the operation immediately; successful admitted turns transfer every current, reply, and media attachment into verified durable spool/materialized storage, rewrite prompt paths, and only then release the original files.
- `ASYNC-UPDATE-01`: public update handlers reject independently with id/category-only diagnostics. `deleted_business_messages` mutation is scoped to the effective profile, exact chat and business connection, and the thread represented by current ownership; message-id collisions outside that scope remain untouched.
