...
- Runtime diagnostics are append-only profile+instance segments with a unique writer generation and lease; scope changes and rotations append boundary records instead of resetting shared files. Retention is exactly 14 days, 100 segments, and 100 MiB per profile including live segments. Stale leases are reclassified through process liveness, dead segments may be removed under the profile transaction, live segments are never deleted, and capacity overflow drops new diagnostics while retaining an in-memory incident. Directories/files use `0700`/`0600` where supported, and persisted values remain redacted.
...
`tmp/telegram/logs*.segment-*.jsonl` is profile+instance-scoped redacted runtime evidence. Each writer owns a unique generation segment and lease; lifecycle changes append boundaries rather than resetting shared evidence. Segments remain diagnostics only and never become routing/provisioning authority.
