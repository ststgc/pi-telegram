...
Operator waiver recorded 2026-07-23: canonical S0 PR `ststgc/pi-telegram#1` and the pre-P0 S0 contract correction PR `#2` may merge without hosted GitHub Actions because the newly created fork had no registered workflow runs and the operator explicitly chose local-only validation for this S0 phase. Required replacement evidence is clean Node `22.22.3` `npm ci` + full `npm run validate`, focused tests, 20/20 seeded race repetitions where applicable, Domain DAG, private tarball/package inspection, and independent no-finding review. This waiver applies only to S0 PRs #1-2; P0, P1, `dev -> main`, exact-tag release, and the completion audit retain their hosted OS-matrix requirements unless separately reviewed and explicitly changed.
